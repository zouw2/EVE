...
#' @param df agrregated prevalidation dataframe from getResults function
#' @param average whether to use prevalidation or compute score for each CV

getHR <- function(df, average = TRUE){
  if(average){
    df %>% 
      group_by(seed, size) %>% 
...
getRMSE <- function(df, label_name, average = TRUE){
  if(average){
    df %>% 
      group_by(seed, size) %>% 
...
getScores <- function(df, label_name, average = TRUE, 
...
#' @param average indicate whether to average scores across CVs per seed
#' 
#' @import ggplot2
#' 
plotHR <- function(df, average = TRUE, Brier = FALSE){
  df.scores <- getHR(df, average)
  ...
  if(average){
    plt1 <- df.scores %>% 
      ...
      geom_boxplot() +
      theme_bw() +
      ylab("Averaged score across seeds") +
      xlab("# feature retain") +
      ggtitle('Averaged scores during RFE') +
...
plotRMSE <-  function(df, label_name, average = TRUE){
  
  df.scores <- getRMSE(df, label_name, average = average)
...
#' @import ggplot2
#' 
plotScores <- function(df, label_name, average = TRUE,
                       metrics = c("Precision", "Recall", "F1", "ROCAUC", "Accuracy"),
                       calibration = FALSE){
  ...
  df.scores <- getScores(df, label_name, average = average, metrics = metrics, calibration)
