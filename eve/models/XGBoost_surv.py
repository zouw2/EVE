...
if not featureList:
    ...
    col2drop = [label, sampleID, col_survival, col_event, "USUBJID", "OS", "OS.event", "PFS", "PFS.event"]
...
event_flag = np.array([i if i == 1 else -1 for i in df.loc[:,col_event]])
y = y.assign(col_surv_mod = y.col_surv * event_flag)

## xgboost hyper-params
xgb_params = {
    "n_estimators": 1000,
    "learning_rate": 0.002, # 0.3, [0,1], usually 0.01 - 0.2
...
for train_index, test_index in ss.split(x):
    ...
    if (cv_id_curr == 0):
      ...
      df_vimp_tmp, df_grid_tmp, df_prevalid_tmp \
          = unit_train(xgbc, X_train, Y_train, X_test, Y_test, sizes, evalm, "surv", RFE_criteria)
...
out_vimp = outP +"/xgb_vimp_"+col_survival+"_seed"+str(seed)+"_cv"+str(cv_id_curr)+".csv"
out_preval = outP +"/xgb_prevalidation_"+col_survival+"_seed"+str(seed)+"_cv"+str(cv_id_curr)+".csv"
out_grid = outP +"/xgb_gridsearch_"+col_survival+"_seed"+str(seed)+"_cv"+str(cv_id_curr)+".csv"
