...
# top features based on frequency of being included in the model
if(nrow(df.vimp)) { # no next door p value when we tune alpha, the previous plotting function still works
#  df.vimp.plt <- plotVIMP2(df.vimp, bin = 100, top_n = 20)

 df.vimp.plt <- plotCoef(df.vimp, top_n = 20, top_n_by = "freq")
## plot
print( df.vimp.plt$plt.dist )
...
library(pheatmap)
# next door analysis is not available when we tune alpha. Here is a temporary fix to rename variable names

colnames(df.vimp.plt$df) <- gsub('coef.avg','vimp', colnames(df.vimp.plt$df))
