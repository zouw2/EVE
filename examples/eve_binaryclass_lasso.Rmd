...
#  barplot(top_f, horiz =T, las=2, main='Number of times a feature is used')
#  cat('(currently only Lasso has this graph)')

  c1 <-  countUniqueFeatures(df.vimp)

  cat('summary of numer of features used in', nrow(c1),'seeds and', ncol(c1),'CVs')
...
if(nrow(df.vimp)) {
 # df.vimp.plt <- plotVIMP2(df.vimp, bin = 100, ft_name = names(top_f) )

df.plt <- plotCoef(df.vimp,ft_name = names(top_f)  )
print( df.plt$plt.features ) # consistent with earlier output  
  ...
  if(F){
  # top by coef, not recommended 
    df.plt <- plotCoef(df.vimp,top_n_by="coef"  )
  ...
  set.seed(27519)
  with(df.plt$top_df,  wordcloud::textplot(x = n, y = vimp, words=feature, show.lines = T, new=T, xlab= 'Frequency (summed across CVs/seeds)', ylab = "Gain (averaged across seeds)"))
