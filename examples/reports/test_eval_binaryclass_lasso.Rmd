...
```{r, message=FALSE, echo=FALSE, warning=FALSE}
with(as.data.frame(df.vimp), hist(table(paste(cv, seed)), 
                                  xlab = 'number of retained features in one run', 
                                  main=paste('summary of model size across', length(unique(paste(cv, seed))),'runs')))
```
