...
with(subset(as.data.frame(df.preval, seed == 1001)), plot(y = col_surv, x = pred, pch=ifelse(col_event == 1, 16, 1), main = 'prediction under seed 1001' ))
...
#df.vimp.plt <- plotVIMP2(df.vimp, bin = 100, top_n = 10)
  ...
  print( df.plt$plt.features )  
    ...
    set.seed(27519)
  with(subset(df.plt$top_df, !is.na(vimp)),  wordcloud::textplot(x = n, y = vimp, words=feature, show.lines = T, new=T, xlab= 'Frequency (summed across CVs/seeds)', ylab = "Gain (averaged across seeds)"))
}
```
