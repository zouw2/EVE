...
if(grepl("\\.r$|\\.R$", runSpec$engine)){
  ...
  df.vimp   <- getResults(project_home, project_name, "rdata", objName = "df_vimp", requireCPrevalidation=F)
} else {
  df.preval <- getResults(project_home, project_name, "prevalidation")
  df.vimp   <- getResults(project_home, project_name, "vimp", requireCPrevalidation=F)
}
...
```{r, echo=FALSE}
correlation(df.preval, 1003) # the scatter plot for seed 1001 and 1002 has a column of dots: many obs seem to be predicted to have the same value,which is strange
```
